...
CKPT_PATH    = os.path.join(OUTPUTS_DIR, 'densenet121_best.pth')
REPORT_TXT   = os.path.join(OUTPUTS_DIR, 'densenet121_metrics.txt')
HIST_ACC_PNG = os.path.join(OUTPUTS_DIR, 'densenet121_history_acc.png')
HIST_LOSS_PNG= os.path.join(OUTPUTS_DIR, 'densenet121_history_loss.png')
CM_PNG       = os.path.join(OUTPUTS_DIR, 'densenet121_confusion_matrix.png')
...
def main():
    ...
    model.classifier = nn.Sequential(nn.Linear(in_f, 512), nn.ReLU(True), nn.Dropout(0.5), nn.Linear(512, len(class_names)))
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        print(f"DenseNet121 | Epoch {epoch:02d}/{EPOCHS} | Train acc {tr_acc:.4f} loss {tr_loss:.4f} | Val acc {va_acc:.4f} loss {va_loss:.4f}")

        if va_acc > best_acc:
            best_acc = va_acc
            torch.save(model.state_dict(), CKPT_PATH)
            print(f"  ✅ Mejor DenseNet guardado → {CKPT_PATH} (val_acc={best_acc:.4f})")
    ...
    plt.imshow(cm, interpolation='nearest'); plt.title('Matriz de Confusión - DenseNet121'); plt.colorbar()
    ...
    print(f"📝 Métricas DenseNet → {REPORT_TXT}")
